...
if ( ! function_exists('get_phrase'))
{
	...
	// common session guard used by every user type (admin, teacher, student, parent, hrm, hostel, accountant, librarian)
	if (! function_exists('check_login_session')) {
	  function check_login_session($user_type) {
		$CI =& get_instance();
		$CI->load->library('session');
		if ($CI->session->userdata($user_type . '_login') != 1) {
